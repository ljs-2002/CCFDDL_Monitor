...
- **🔍 交互式即时查询**：通过 GitHub Discussions 或手动触发，随时调取特定会议的**基础信息和深度报告**。
- **📢 多通道精准通知**：支持 **PushPlus 微信推送**（Markdown 渲染）和 **SMTP 邮件**。
- **📊 功能自主切换**：可以通过 Secrets 中配置 `USE_LLM` 环境变量来启用或禁用 LLM 功能。
...
| 变量名 | 说明 | 示例 |
| :--- | :--- | :--- |
| `LLM_API_KEY` | LLM 的授权密钥，如果启用 LLM 功能则必填 | `sk-xxxx...` |
...
| 变量名 | 说明 | 默认值 | 示例/建议 |
| :--- | :--- | :--- | :--- |
| `USE_LLM` | 是否启用 LLM 功能 | `True` | `True` 或 `False`，设为 `False` 则仅做会议变动监控，不进行论文分析 |
| `LLM_BASE_URL` | LLM API 的基础地址，不启用LLM功能可不填 | `https://api.openai.com/v1` | `https://api.deepseek.com/v1` |
| `LLM_MODEL` | 使用的模型名称，不启用LLM功能可不填 | `gpt-3.5-turbo` | `deepseek-chat`, `gpt-4o` |
...
>
> 💡 若不启用LLM功能，但是此前已经通过 LLM 生成过部分会议的分析，则这些会议在推送时仍然会返回已有的分析数据。
...
3. **会议查询**：
    1. **GitHub Discussions 查询**（推荐，支持手机 App）
        ...
        - **发起查询**：在 **Query** 分类下发起新讨论，标题格式为：`q: 会议简称`（例如：`q: aaai`）。
           - 若使用 `qn: 会议简称`（例如：`qn: icml`），则表示不启用 LLM，仅查询基础信息，若数据库中无相关会议信息则只会从ccfdeadlines中获取基础信息并返回，不会执行 LLM 分析。
